...
$queryforcat = "SELECT * FROM blogcategories WHERE `caturl`='$pageurl'";
$queryforcat_res = mysqli_query($conn, $queryforcat);
$rowforcat = mysqli_fetch_assoc($queryforcat_res);
$catid1 = $rowforcat['blogs_id'];

$queryforblog = "SELECT * FROM blogs INNER JOIN blogcategories ON blogcategories.blogs_id = blogs.blogcategories  WHERE `shownavbar` = 'Yes' AND `blogcategories`= $catid1 ORDER BY comment_count DESC ";
                ...
                <div class="card border-0 pb-3">
                  <a href="http://localhost/hyper/'.$rowforblog['pageurl'].'">
                   <div class="img-wraper rounded">
                      <img class="card-img-top inner-img rounded pb-2" src="admin/'. $rowforblog['img'] .'" alt="Card image" style="width:100%">
                   </div>
